...
function GallerySlide({ onNext }) {
  ...
  const photos = [
    { id: 3, span: "col-span-1 md:col-span-1 order-2 md:order-1", text: "Endless Joy" },
    { id: 10, span: "col-span-2 md:col-span-2 order-1 md:order-2", text: "Perfect Days" },
    { id: 5, span: "col-span-1 md:col-span-1 order-3 md:order-3", text: "Forever" }
  ];
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 w-full mt-2 md:mt-4">
          {photos.map((photo, i) => (
            <motion.div
              ...
              className={`relative overflow-hidden rounded-2xl md:rounded-3xl border-3 md:border-4 border-white shadow-[0_15px_40px_rgba(255,182,193,0.6)] group cursor-pointer bg-white ${photo.span} flex flex-col`}
            >
              {/* Photo */}
              <div className="relative w-full aspect-[4/3] md:aspect-auto md:flex-1 overflow-hidden min-h-[120px] md:min-h-[160px] bg-rose-50">
                <img 
              ...
              <div className="w-full bg-white py-2 px-2 md:py-3 md:px-4 text-center border-t border-rose-100 flex items-center justify-center">
                <p className="text-[#800020] font-serif text-sm md:text-xl font-bold tracking-wider">
                  {photo.text}
                </p>
              </div>
            </motion.div>
